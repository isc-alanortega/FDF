...
{
    public partial class UserManeuverSimplexExpo
    {
        #region INJECTS
        ...
        #region PROPERTYS
        private GridListPager<ManeuversCustomsExpoData> PagerReference { get; set; }
        private IList<ManeuversCustomsExpoData>? ManeuversCustoms { get; set; }
        ...
        private bool IsCreditCustomer { get; set; } = false;
        private Dictionary<long, bool> ExpandedCardStates = new Dictionary<long, bool>();
        private Dictionary<int, ManeuversContainerInfoExpo> ContainerResponseStates = new Dictionary<int, ManeuversContainerInfoExpo>();
        ...
        public async void OnClickOpenPay(ManeuversCustomsExpoData maneuverData)
        {
        ...
        {
            var response = await DataAccessService.GetManeuversCustomersCimaSimplexExpoAsync(requestGrid: Request);

            (List<ManeuversCustomsExpoData> data, int total) = (response is null || !response.Operation_Succeeded || response.Result is null)
                ? (new List<ManeuversCustomsExpoData>(), 0)
        ...
        private async Task ToggleExpandedCard(int webRequestId, string tipoOperacion)
        {
            try
            {
                if (!ContainerResponseStates.ContainsKey(webRequestId))
                {
                    var response = await DataAccessService.GetInfoContainerCimaSimplexExpo(webRequestId, tipoOperacion);
                    if (response?.Result != null)
                        ContainerResponseStates[webRequestId] = response.Result; // Almacenar el objeto completo
                    else
                        ContainerResponseStates[webRequestId] = new(); // Objeto vacío si no trae nada
                }
